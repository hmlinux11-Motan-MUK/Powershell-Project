...
$CertificatePath  = "M:\PS project\tenantassesmentcert.pfx"
$CertPlainPassword = "@Cert!"
$OutputFolder     = "M:\PS project\Result\OneDriveTest"
...
$sampleItems | Export-Csv -NoTypeInformation -Path (Join-Path $OutputFolder "SampleItems_$TimeStamp.csv")
...
if ($permResults.Count -gt 0) {
    ...
    $permResults | Export-Csv -NoTypeInformation -Path (Join-Path $OutputFolder "Permissions_$TimeStamp.csv")
}
else {
...
try {
    ...
    if ($linkResults.Count -gt 0) {
        ...
        $linkResults | Export-Csv -NoTypeInformation -Path (Join-Path $OutputFolder "SharingLinks_$TimeStamp.csv")
    }
    else {
